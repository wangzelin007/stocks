...
def initialize(context):
    ...
    set_order_cost(
        OrderCost(open_tax=0, close_tax=0.001, open_commission=0.0003, close_commission=0.003, min_commission=5),
        type='stock')
    # 个股最大持仓比重
...
def risk_management(context):
    ...
    index_stoploss_diefu(context, 3, 0.1, g.open_sell_securities)
    portfolio_stoploss(context, 0.1, g.open_sell_securities)
    ### _风控函数筛选-结束 ###
    return
...
def check_stocks(context):
    if g.check_stocks_days % g.check_stocks_refresh_rate != 0:
        ...
        return
    # 设定股票池
    g.security_universe = get_index_stocks('000905.XSHG')
    # 股票池赋值
...
def trade(context):
    ...
    if g.buy_trade_days % g.buy_refresh_rate == 0:
        ...
        ### _入场函数筛选-开始 ###
        buy_lists = [security for security in buy_lists if ATR_judge_xiaoyu(security, 1, 14)]
        ### _入场函数筛选-结束 ###
...
def financial_statements_filter(context, security_list):
    ### _财务指标筛选函数-开始 ###
    security_list = financial_data_filter_dayu(security_list, indicator.gross_profit_margin, 70)
    ### _财务指标筛选函数-结束 ###
...
def sell(context, buy_lists):
    ...
    ### _出场函数筛选-开始 ###
    sell_lists = [security for security in sell_lists if n_day_chg_xiaoyu(security, 10, 0.05)]
    ### _出场函数筛选-结束 ###

    # 卖出股票
    if len(sell_lists) > 0:
        for stock in sell_lists:
            sell_by_amount_or_percent_or_none(context, stock, g.sell_by_amount, g.sell_by_percent,
                                              g.open_sell_securities)
